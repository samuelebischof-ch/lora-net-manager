    ...
    .subscribe(res => {
      ...
      // new data
        ...
        for (let i = 0; i < res.newData.length; i++) {
          const hasSensor = (res.newData[i].hasSensor !== undefined) ? res.newData[i].hasSensor : false;
          const newTempData = res.newData[i].data;
          if (hasSensor) {
            _data.data[i].data[0].data.push(newTempData);
          }
        }
